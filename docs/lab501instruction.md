**[Please Follow Steps Below To Execute this Hands On Lab Within Your Environment]**

1. Navigate to https://github.com/microsoft/ignite25-LAB501-modernizing-aspnet-applications-with-azure-migrate-and-github-copilot/tree/main/src/ARM . Download the ARM template and associated Parameters file. We will use these files to deploy all Azure Resources required to complete this exercise.
2. Once the files are downloaded open the "labresources.parameters.json" file and provide value for "sqlAdministratorLoginPassword" and "sqladminUsername". This is the username and password for Azure SQL Database. Save the file.
3. Logon to Azure Portal and create a resource group named "ignite2025" . This lab assumes that all Azure Resources and Managed Instance on Azure App Service Plan are deployed to the same Azure Region. Select an Azure Region where Managed Instance on Azure App Service is currently available . Please refer to https://learn.microsoft.com/en-gb/azure/app-service/quickstart-managed-instance?tabs=portal#prerequisites for list of regions. (The Azure region availability list will continue to be edited as we deploy Managed Instance on Azure App Service to additional Azure regions.)
4. Once the resource group is created you should start deploying the ARM template and associated paramater file. Refer to this document https://learn.microsoft.com/en-us/azure/azure-resource-manager/templates/template-tutorial-use-parameter-file?tabs=azure-cli for deploying the ARM template. Use the resource group name as "ignite2025".
5. Once all the Azure resources are deployed logon to the Azure Portal. Navigate to resource group "ignite2025". Now navigate to Azure SQL Database named autogeneratedprefix/l501devshopdb. Click "Query editor (preview)" and login using the username and password you provided in step 2 above.
6. Download SQL scripts to create tables and populate sample data from https://github.com/microsoft/ignite25-LAB501-modernizing-aspnet-applications-with-azure-migrate-and-github-copilot/tree/main/src/SQL
7. Now execute "CreateTables.sql" within the "Query editor" to create required tables. Now execute "PopulateTables.sql" to populate these tables with sample data. (You may also use any tool of your choice to connect to Azure SQL Database and execute these SQL scripts.).
8. Since you are executing this exercise in your own environment You should skip the instructions on how to connect GitHub Account with Visual Studio 2026.
9. Please ensure that you are using Visual Studio 2026 for this exercise and it's connected  with your Azure Subscription and GitHub Credentials.
10. You should download the code locally for sample app that we would publish in later steps. (https://github.com/microsoft/ignite25-LAB501-modernizing-aspnet-applications-with-azure-migrate-and-github-copilot/tree/main/src )
11. You should now navigate to the section **[Run App Modernization Assessment (GitHub Copilot app modernization for .NET)]** and follow the steps as mentioned.
